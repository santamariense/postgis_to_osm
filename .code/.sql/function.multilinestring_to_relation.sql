...
-- MULTILINESTRING to relation
DROP FUNCTION IF EXISTS postgis_to_osm.multilinestring_to_relation;
CREATE OR REPLACE FUNCTION postgis_to_osm.multilinestring_to_relation(
  geom geometry,
  fields TEXT[][]
) RETURNS BIGINT AS $$
DECLARE
  linestring_geometry geometry;
  linestring_id BIGINT;
...
BEGIN
  ...
  FOR i IN 1..COALESCE(array_length(field_map, 1), 0) LOOP
    IF field_map[i][1] = 'type' THEN
      IF field_map[i][2] IS NULL OR field_map[i][2] = '' THEN
        field_map[i][2] := 'collection'; -- Always 'collection' for line collections
      END IF;
      type_found := TRUE;
      EXIT;
    END IF;
  ...
  -- Check the number of geometries (linestrings in the collection)
  num_geometries := ST_NumGeometries(geom);
  IF num_geometries IS NULL OR num_geometries = 0 THEN
    RAISE EXCEPTION 'Geometry must contain at least one linestring.';
  END IF;

  -- Loop through each linestring in the geometry collection
  FOR i IN 1..num_geometries LOOP
    linestring_geometry := ST_GeometryN(geom, i);

    -- Use linestring_to_way function to insert and retrieve ID for each linestring
    linestring_id := postgis_to_osm.linestring_to_way(
      linestring_geometry,
      ARRAY[ARRAY['','']]
    );
    ...
    new_members := new_members || ARRAY[
      ARRAY['type', 'way'],
      ARRAY['ref', linestring_id::TEXT],
      ARRAY['role', ''] -- No role detection for linestrings
    ];
  END LOOP;
...
/*
-- Test zone
SELECT postgis_to_osm.multilinestring_to_relation( 
  ST_GeomFromText('MULTILINESTRING(
  (51.155950 29.855570, 51.157950 29.856570, 51.159950 29.857570),
  (51.160950 29.860570, 51.162950 29.861570, 51.164950 29.863570),
  (51.165950 29.865570, 51.167950 29.866570, 51.169950 29.867570),
  (51.170950 29.868570, 51.172950 29.869570, 51.174950 29.870570)
)
